...
require 'spec_helper'
require 'rfacter/util/normalization'

describe RFacter::Util::Normalization do
  ...
  describe "validating strings" do
    describe "and string encoding is supported", :if => String.instance_methods.include?(:encoding) do
      ...
      it "rejects strings that are not UTF-8 and do not match their claimed encoding" do
        ...
        }.to raise_error(RFacter::Util::Normalization::NormalizationError, /String encoding Shift_JIS is not UTF-8 and could not be converted to UTF-8/)
      end
      ...
      it "rejects strings that claim to be UTF-8 encoded but aren't" do
        ...
        }.to raise_error(RFacter::Util::Normalization::NormalizationError, /String.*doesn't match the reported encoding UTF-8/)
      end
    end
    ...
    describe "and string encoding is not supported", :unless => String.instance_methods.include?(:encoding) do
      ...
      it "rejects strings that are not UTF-8" do
        ...
        expect {
          subject.normalize(str)
        }.to raise_error(RFacter::Util::Normalization::NormalizationError, /String .* is not valid UTF-8/)
      end
    end
  end
  ...
  [:sym, Object.new, Set.new].each do |val|
    it "rejects #{val.inspect}:#{val.class}" do
      ...
      }.to raise_error(RFacter::Util::Normalization::NormalizationError, /Expected .*but was #{val.class}/ )
    end
  end
end
